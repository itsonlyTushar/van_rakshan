...
function Form() {
  const errorData = useSelector((state) => state.teamSlice.errors);
  ...
  const handleSubmit = async (e) => {
    ...
    kys.forEach((fld) => {
      dispatch(validations({ field: fld }));
    });
  ...
  const handleChange = (e) => {
    ...
    dispatch(validations({ field: e.target.name }));
  };
  ...
  return (
    <>
      ...
      <Modal show={close} setShow={setClose} message={`You have submitted your data, can't submit again`} />

      <div className="flex flex-col">
        <div data-aos="fade-up">
        <CounterAnimation animation={"zoom-in-up"} count={300} label="Volunteers" /> 
        </div>
        <div data-aos="fade-up">
        <CounterAnimation animation={"zoom-in-up"} count={50} label="Programs" />
        </div>
        <div data-aos="fade-up"> 
        <CounterAnimation animation={"zoom-in-up"} count={1000} label="Species" />
        </div>
      </div>
    </>
  );
